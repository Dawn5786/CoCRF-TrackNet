from trackprocess.tracker.base import BaseTracker
import torch
import torch.nn.functional as F
import math
import time
from trackprocess import dcf, TensorList
from trackprocess.features.preprocessing import numpy_to_torch
from trackprocess.utils.plotting import show_tensor, plot_graph, draw_figure
from trackprocess.features.preprocessing import sample_patch_multiscale, sample_patch_transformed
from trackprocess.features import augmentation
import matplotlib.pyplot as plt
import numpy
import os

# from tensorboardX import SummaryWriter
# from pytracking.parameter.dimp.dimp18 import parameters
# from pytracking.utils.plotting import  show_tensor


class CoCRF(BaseTracker):
    ...
    def initialize(self, image, info, omit: dict) -> dict:
        # Initialize some stuff
        # self.frame_num = 1
        self.frame_num = omit + 1
        ...
        # Set threshold L, A, angle-deta
        self.L_farT = math.sqrt(math.pow(self.target_sz[0], 2) + math.pow(self.target_sz[1], 2))
        self.A_nearT = math.pow(self.target_sz[1], 1)
        self.beta_angleT = (math.atan(self.target_sz[1]/self.target_sz[0]))/2
        ...
        ## tensorboardX net
        # with SummaryWriter(comment='DiMP_try') as w:
        #     # w.add_graph(self.net, (im, ))
        #     w.add_graph(self.net.net, (torch.randn(1, 1, 3, 288, 288).cuda(), torch.randn(1, 1, 3, 288, 288).cuda(), torch.randn(1, 1, 4).cuda(),
        #             torch.randn(1, 1, 2, 4).cuda()))
            # w.add_graph(self.net.get_backbone_bbreg_feat, (init_backbone_feat, ))
            # w.add_graph(self.net.bb_regressor.get_iou_feat, (self.get_iou_backbone_features(init_backbone_feat), ))
            # w.add_graph(self.net.bb_regressor.get_modulation, (self.init_iou_net(init_backbone_feat), ))

        out = {'time': time.time() - tic}
        return out, self.L_farT, self.A_nearT, self.beta_angleT


    def track(self, image, redet_flag, omega=None, last_box=None) -> dict:
    # def track(self, image) -> dict:
        self.debug_info = {}

        if bool(1-redet_flag):
            self.frame_num += 1

        self.debug_info['frame_num'] = self.frame_num

        # writer = SummaryWriter(comment='DiMP_guan')
        # writer.add_graph(self.net, (, ))  # im, test_x, target_box, sample_coords
        # writer.close()
        # Convert image
        im = numpy_to_torch(image)

        ####### redect judge & decay
        if redet_flag and omega != None:
            # last_box
            # neibour_box = [ bo *2.2 for bo in last_box] # neibour_box x, y, w, h
            left_board = last_box[0] - 0.6*last_box[2]  # 0.6 = 2.2/2 -0.5
            right_board = last_box[0] + 1.6*last_box[2]  # 1.6 = 1 +(2.2/2 -0.5)
            up_board = last_box[1] - 0.6*last_box[3]
            down_board = last_box[1] + 1.6*last_box[3]

            decay_mask = numpy.zeros(image.shape[0:2])
            decay_mask[:] = 0.5*omega
            decay_mask[int(up_board):int(down_board), int(left_board):int(right_board)] = 1

            decay_mask_tensor = torch.from_numpy(decay_mask).float()
            im = im * decay_mask_tensor
        #######################################
        # ------- LOCALIZATION ------- #
        ...
        backbone_feat, sample_coords = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        # writer.add_graph(self.net.extract_backbone, (, ))  # im, test_x, target_box, sample_coords
        ...
        # Draw the response map
        # scores_press = scores_raw.squeeze(dim=0).squeeze(dim=1)
        # scores_press = scores_raw.squeeze().cpu().clone().detach().numpy()
        # plt.cla()
        # plt.imshow(scores_press)
        # plt.axis('off')
        # plt.axis('equal')
        # bug_path = '/data1/lxt/2021remote_tracking_code/demo/trackprocess/tracking_results/CoCRF/hotmap'
        # if not os.path.exists(bug_path):
        #     os.makedirs(bug_path)
        # fig_file = '{}/{}'.format(bug_path, self.frame_num)
        # fig_file = '{}.jpg'.format(fig_file)
        # Hotmap = plt.gcf()
        # Hotmap.savefig(fig_file)
        # show_tensor(scores_press)
        ...
        # out = {'target_bbox': new_state.tolist(), 'score_max': scores_press.max()}
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        # writer = SummaryWriter(comment='DiMP_backbone')
        # writer.add_graph(self.net.extract_backbone(im_patches))
        # writer.close()
    ...
    def get_iou_features(self, backbone_feat):
        with torch.no_grad():
            feat_t = self.get_iou_backbone_features(backbone_feat)
            iou_feat = self.net.bb_regressor.conv4_2t(self.net.bb_regressor.conv4_1t(feat_t[0]))
            return iou_feat
            # return self.net.bb_regressor.get_iou_feat(self.get_iou_backbone_features(backbone_feat))

    def get_feat1_roi(self, iou_backbone_feat, target_boxes):
        with torch.no_grad():
            return self.net.bb_regressor.get_roi_featr(iou_backbone_feat, target_boxes)
    ...
    def init_iou_net(self, backbone_feat):
        ...
        self.feat1_roi = self.get_feat1_roi(iou_backbone_feat, target_boxes)
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        # iou_features = TensorList(iou_features[scale_ind:scale_ind+1,...])
        ...
        # Optimize the boxes
        # backbone_feat = TensorList(backbone_feat[:])
        # iou_backbone_feat = TensorList([x[:target_boxes.shape[0],...] for x in iou_backbone_feat])
        iou_backbone_feat = self.get_iou_backbone_features(backbone_feat)
        output_boxes, output_iou = self.optimize_boxes(iou_backbone_feat, init_boxes)
    ...
    def optimize_boxes(self, iou_bone_feat, init_boxes):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            # bb_init.requires_grad = False

            num_sequences = bb_init.shape[0]
            num_proposals = bb_init.shape[1]

            # with torch.no_grad():
            feat2_roi = self.net.bb_regressor.get_roi_featt(iou_bone_feat, bb_init)
            feat2_roi_atten = self.net.bb_regressor.get_coatten_feat(self.feat1_roi, feat2_roi, num_proposals)
            outputs = self.net.bb_regressor.predict_iou(feat2_roi_atten, num_proposals, num_sequences)
