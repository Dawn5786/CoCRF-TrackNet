...
class ATOM(BaseTracker):
    ...
    def track(self, image) -> dict:
        ...
        features_channels = test_x.squeeze(dim=0)
        bug_path = '/home/lxt/project/pytracking_B/pytracking/tracking_results/atom/features_channels'
        fig_file = '{}/{}'.format(bug_path, self.frame_num)
            #fig_file = '{}.txt'.format(fig_file)
        features_to_save = features_channels[0].cpu()
        for cc in  range(features_to_save.shape[0]):
            file = '{}-{}.txt'.format(fig_file, cc)
            numpy.savetxt(file, features_to_save[cc,:,:], delimiter=',', fmt="%6.8f")

        # Compute scores
        scores_raw = self.apply_filter(test_x)

        # Draw the response map
        # scores_press = scores_raw.squeeze(dim=0)
        # scores_press = scores_press.squeeze(dim=0)
        # scores_press = scores_raw.squeeze(dim=0)
        # scores_press = scores_press.squeeze(dim=0)
        # show_tensor(scores_press)

        # bug_path = '/home/lxt/project/pytracking_B/pytracking/tracking_results/atom/hotmap_txt'
        # fig_file = '{}/{}'.format(bug_path, self.frame_num)
        # fig_file = '{}.txt'.format(fig_file)
        # scores_to_save = scores_press[0].cpu()
        # numpy.savetxt(fig_file, scores_to_save, delimiter=',', fmt="%6.8f")
