...
# from trackprocess.parameter.dimp288_channel_atten_redet.dimp288_channel_atten_redet import parameters
from trackprocess.parameter.CoCRF.cocrf import parameters
# from pytracking.parameter.dimp288_channel_atten_redet_D.dimp288_channel_atten_redet_D import parameters
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None):
        ...
        if self.run_id is None:
            self.results_dir = '{}/{}/{}'.format(env.results_path, self.name, PM18.model)
            self.visual_dir = '{}/{}/{}'.format(env.visual_path, self.name, PM18.model)
        else:
            self.results_dir = '{}/{}/{}_{:03d}'.format(env.results_path, self.name, PM18.model, self.run_id)
            self.visual_dir = '{}/{}/{}_{:03d}'.format(env.visual_path, self.name, PM18.model, self.run_id)
        if not os.path.exists(self.results_dir):
            os.makedirs(self.results_dir)
            os.makedirs(self.visual_dir)

        # tracker_module = importlib.import_module('pytracking.tracker.{}'.format(self.name))
        tracker_module = importlib.import_module('trackprocess.tracker.{}'.format(self.name))
        self.tracker_class = tracker_module.get_tracker_class()


    def run(self, seq, visual_path, visualization=None, debug=None, visdom_info=None):
    # def run(self, seq, visualization=None, debug=None, visdom_info=None):
        ...
        output = tracker.track_sequence(seq, visual_path)
    ...
    def get_parameters(self):
        ...
        param_module = importlib.import_module('trackprocess.parameter.{}.{}'.format(self.name, self.parameter_name))
